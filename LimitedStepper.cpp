...
void PiStepper::homeMotor() {
    // Move towards a limit switch until it is activated
    // For simplicity, moving in one direction is assumed
    // This should be adapted based on your specific setup
    enable();
    const int direction = 0; // Adjust based on your setup
    while (gpiod_line_get_value(limit_switch_top) == 0) {
        moveSteps(1, direction);
    }
    _currentStepCount = 0; // Reset step counter at home position
    disable();
}
